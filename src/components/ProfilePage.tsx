...
const ProfilePage: React.FC<ProfilesDetailsProps> = ({ name, uuid }) => {
  ...
  const { attendeeInfo, isLoadingAttendeeInfo } = useAttendeeInfo(userId, uuid);
  const { goodToKnow, isLoadingGoodToKnow } = useGoodToKnow(userId, uuid);
  const { actionItemsResponse, isLoadingActionItems } = useActionItems(userId, uuid);
  const { salesCriteria, isLoadingSalesCriteria } = useSalesCriteria(userId, uuid);
  const { kpi, actionItems } = actionItemsResponse ? actionItemsResponse : {};
  const { data, isLoading, error } = useStrengthsAndCategories(userId, uuid);
  const { profile_category, strengths } = data ? data : {};
  const { workExperience, isLoadingWorkExperience } = useWorkExperience(userId, uuid);
